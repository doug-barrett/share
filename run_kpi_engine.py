...
try:
    ...
    sv_hist_sql = f"""
    WITH key_lookup AS (
        SELECT DISTINCT kpi_id, kpi_dim_id, kpi_engine_key, execution_level,
               dim_name, category, subcategory, kpi_name
        FROM {config.output_table}
       
    ),
    hist_bf_ranked AS (
        SELECT kpi_id, kpi_dim_id, date_key, scenario, value,
               ROW_NUMBER() OVER (PARTITION BY kpi_id, kpi_dim_id, date_key, scenario ORDER BY version_id DESC) AS _rn
        FROM {config.budget_forecast_table}
        WHERE date_key != '{reporting_month}'
          AND kpi_dim_id RLIKE '^[0-9]+$'
    ),
    hist_bf AS (
        SELECT kpi_id, kpi_dim_id, date_key, scenario, value
        FROM hist_bf_ranked WHERE _rn = 1
    ),
    hist_bf_pivot AS (
        SELECT
            kpi_id, kpi_dim_id, date_key,
        ...
        FROM hist_bf
        GROUP BY kpi_id, kpi_dim_id, date_key
    ),
    hist_actuals AS (
        SELECT *,
               CAST(kpi_dim_id AS BIGINT) AS kpi_dim_id_long
    ...
    INNER JOIN key_lookup kl ON ha.kpi_id = kl.kpi_id AND ha.kpi_dim_id_long = kl.kpi_dim_id
    LEFT JOIN hist_bf_pivot hbf
        ON ha.kpi_id = hbf.kpi_id
       AND ha.kpi_dim_id_long = CAST(hbf.kpi_dim_id AS BIGINT)
    ...
    _MERGES = [
        ...
        {'df': sv_df, 'table': 'scr_variance', 'id_col': 'var_id',
         'merge_keys': ['kpi_engine_key', 'month_date'],
         'update_cols': ['variance_id', 'kpi_ref', 'kpi_aggregation', 'kpi_category',
                         'kpi_subcategory', 'unit_of_measure', 'location_name',
